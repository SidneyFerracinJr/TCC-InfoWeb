...
class Mapos extends MY_Controller
{
    ...
    public function editarLogo()
    {
        ...
        if ($retorno) {
            $this->session->set_flashdata('success', 'A logomarca foi alterada com sucesso.');
            log_info('Alterou a logomarca do emitente.');
        } else {
            $this->session->set_flashdata('error', 'Ocorreu um erro ao tentar alterar a logomarca. Tente novamente!');
        }
        redirect(site_url('mapos/emitente'));
    }
    ...
    public function emails()
    {
        if (!$this->permission->checkPermission($this->session->userdata('permissao'), 'cEmail')) {
            $this->session->set_flashdata('error', 'Você não tem permissão para visualizar fila de emails');
            redirect(base_url());
        }
    ...
    {
        if (!$this->permission->checkPermission($this->session->userdata('permissao'), 'cEmail')) {
            $this->session->set_flashdata('error', 'Você não tem permissão para excluir email da fila.');
            redirect(base_url());
        }

        $id = $this->input->post('id');
        if ($id == null) {
            $this->session->set_flashdata('error', 'Erro ao tentar excluir email da fila.');
        ...
        log_info('Removeu um email da fila de envio. ID: ' . $id);
